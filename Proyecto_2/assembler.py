...
def parse_immediate(token: str, line: int) -> int:
    """
    Convierte un inmediato en entero. Soporta decimal y base automática (0x, 0b, etc).
    """
    token = token.strip()
    try:
        return int(token, 0)
...
# ------------------------------------------------------------
# Ensamblador (2 pases: labels + instrucciones)
# ------------------------------------------------------------
def assemble(source: str) -> List[Instruction]:
    ...
    for text, line_num, addr in instruction_records:
        instr = parse_instruction(text, line_num, addr, labels)
        instructions.append(instr)

    return instructions


def parse_instruction(text: str, line: int, addr: int, labels: Dict[str, int]) -> Instruction:
    """
    ...
    if opcode in ("add", "sub", "and", "or"):
        ...
        rs1 = parse_register(tokens[2], line)
        rs2 = parse_register(tokens[3], line)
        return Instruction(
            opcode=opcode,
            rd=rd,
            rs1=rs1,
    ...
    if opcode == "addi":
        ...
        rd = parse_register(tokens[1], line)
        rs1 = parse_register(tokens[2], line)
        imm = parse_immediate(tokens[3], line)
        return Instruction(
            opcode=opcode,
            rd=rd,
            rs1=rs1,
    ...
    if opcode == "lw":
        ...
        rd = parse_register(tokens[1], line)
        imm = parse_immediate(tokens[2], line)
        rs1 = parse_register(tokens[3], line)
        return Instruction(
            opcode=opcode,
            rd=rd,
            rs1=rs1,
    ...
    if opcode == "sw":
        ...
        rs2 = parse_register(tokens[1], line)
        imm = parse_immediate(tokens[2], line)
        rs1 = parse_register(tokens[3], line)
        return Instruction(
            opcode=opcode,
    ...
    if opcode in ("beq", "bne"):
        if len(tokens) != 4:
            raise AssemblerError(f"Línea {line}: sintaxis '{opcode} rs1, rs2, label' esperada")
        rs1 = parse_register(tokens[1], line)
        rs2 = parse_register(tokens[2], line)
        label = tokens[3]
